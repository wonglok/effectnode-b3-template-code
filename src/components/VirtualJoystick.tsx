...
export function VirtualJoystick() {
    ...
    return (
        <div
            className='pointer-events-none absolute bottom-6 right-6 z-40 select-none'
            style={{ width: '150px', height: '150px' }}
        >
            {/* Decorative socket; nipple draws its own translucent disc + knob on top */}
            <div className='pointer-events-none absolute bottom-[calc(-9px)] right-[calc(-8px)] h-[150px] w-[150px] rounded-full border border-tiffany-400/25 bg-studio-900/40 shadow-[0_8px_30px_rgba(0,0,0,0.35)] backdrop-blur-sm' />

            {/* Actual touch zone — fills the overlay, nipple appends its UI here */}
            <div ref={zoneRef} className='pointer-events-auto absolute inset-0 h-full w-full touch-none' />
        </div>
    )
}
