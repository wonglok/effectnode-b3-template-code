...
import { createMotionCatalog, partsFor, variantForUrl } from '../../b3/b3-runtime/src/components/AvatarSDK'
                    ...
                    >
                        ...
                        <div className='flex shrink-0 items-center justify-between gap-3 border-b border-studio-700 px-4 py-3'>
                            <div className='min-w-0'>
                                <h2 className='text-sm font-semibold text-ice-50'>Avatar picker</h2>
                                <p className='text-[11px] text-ice-600'>
                                    Gender, body and head apply to the character instantly — stay motions preview here
                                    only.
                                </p>
                            </div>
                            <button
                            ...
                            <div className='flex min-w-0 flex-1 flex-col gap-3 p-4'>
                                ...
                                <div className='flex flex-col gap-2'>
                                    ...
                                    <div className='flex flex-wrap gap-0.5 rounded-md border border-studio-700 bg-studio-900/60 p-0.5'>
                                        {STAY_MOTIONS.map((m) => (
                                            <Chip key={m.name} on={motion === m.name} onClick={() => setMotion(m.name)}>
                                                {labelOf(m.name)}
                                            </Chip>
                                        ))}
                                    </div>
...
function RowLabel({ children, hint }: { children: string; hint?: string }) {
    return (
        <div className='flex items-baseline justify-between'>
            <span className='text-[10px] font-semibold tracking-[0.28em] text-text-muted uppercase'>{children}</span>
            {hint ? <span className='text-[10px] text-ice-600'>{hint}</span> : null}
        </div>
    )
}
...
function ChipRow({ kind }: { kind: PartKind }) {
    const gender = useAvatarStore((s) => s.gender)
    const url = useAvatarStore((s) => (kind === 'body' ? s.assets.body : s.assets.face))
    ...
    return (
        <div className='flex flex-wrap gap-0.5 rounded-md border border-studio-700 bg-studio-900/60 p-0.5'>
            {variants.map((v) => (
                <Chip key={v.id} on={active?.id === v.id} onClick={() => setAsset(kind, v.url)}>
                    {v.label}
                </Chip>
            ))}
        </div>
