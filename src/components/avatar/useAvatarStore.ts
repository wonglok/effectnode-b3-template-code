...
function defaultRigClips(): Record<LocomotionKey, MotionClipDef> {
    ...
    for (const key of LOCOMOTION_STATES) {
        ...
        out[key] = STAY_CLIPS.find((c) => c.name === name) ?? {
            name,
...
function cloneGenderDefaults(defaults: Partial<Record<Gender, AvatarAssets>>): Partial<Record<Gender, AvatarAssets>> {
...
function findIndex(offsets: AvatarOffsets, bodyUrl: string, faceUrl: string): number {
    return offsets.findIndex((e) => e.bodyUrl === bodyUrl && e.faceUrl === faceUrl)
}
...
function removeEntry(offsets: AvatarOffsets, bodyUrl: string, faceUrl: string): AvatarOffsets {
    const index = findIndex(offsets, bodyUrl, faceUrl)
...
function liveCombo(offsets: AvatarOffsets, bodyUrl: string, faceUrl: string): { body: Offset3; head: Offset3 } {
    const index = findIndex(offsets, bodyUrl, faceUrl)
    if (index >= 0) {
...
function withAxis(offset: Offset3, key: OffsetKey, axis: Axis, value: number): Offset3 {
...
function withGroup(offset: Offset3, key: OffsetKey, value: Offset3[OffsetKey]): Offset3 {
...
export const useAvatarStore = create<AvatarState>()((set, get) => ({
    ...
    genderDefaults: cloneGenderDefaults(DEFAULTS.genderDefaults ?? GENDER_ASSETS),
    assetsByGender: seedAssetsByGender(
    ...
    setGender: (gender) =>
        set((s) => {
            ...
            const next: AvatarAssets = s.assetsByGender[gender] ?? s.genderDefaults[gender] ?? GENDER_ASSETS[gender]
    ...
    setMotionSection: (sectionId) =>
        set((s) => {
            ...
            const activeMotion =
                remembered ?? (section.names.includes(s.activeMotion) ? s.activeMotion : section.default)
            return {
    ...
    copyManifest: () => {
        ...
        const failed = (): void => set({ notice: 'Copy failed — clipboard unavailable' })
    ...
    pickManifestDir: async () => {
        ...
        if (typeof picker !== 'function') {
            set({
                notice: 'Folder picker needs a Chromium browser — use "Export JSON" instead.',
            })
            return
        }
        try {
...
export async function restoreManifestExportDir(): Promise<void> {
    try {
        const dir = await localforage.getItem<FileSystemDirectoryHandle>(EXPORT_DIR_KEY)
        useAvatarStore.setState(dir ? { exportDir: dir, exportDirName: dir.name } : {})
    } catch {
