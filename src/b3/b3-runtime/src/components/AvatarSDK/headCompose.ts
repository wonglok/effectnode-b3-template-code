...
export function classifyHeadCompose(opts: {
    ...
    const hips = findBone(bodyScene, 'mixamorig:Hips') ?? findBone(bodyScene, 'Hips')
    ...
    const headF = bone ? (findBone(faceScene, bone.name) ?? findHeadBone(faceScene)) : null
    ...
    const congruent =
        !!bodyHeadW &&
        !!faceHeadW &&
        Math.hypot(bodyHeadW[0] - faceHeadW[0], bodyHeadW[1] - faceHeadW[1], bodyHeadW[2] - faceHeadW[2]) < 0.01
...
export function createHeadAttachment(opts: {
    ...
    const { bone, faceRigged, congruent, forceSeat, bodyHeadW, bodyNeckW, faceHeadW, faceNeckW } = plan
    ...
    if (faceRigged) {
        if (bone) {
            if (congruent && !forceSeat) {
                ...
                if (faceHeadW) {
                    seat.position.set(-faceHeadW[0] * s, -faceHeadW[1] * s, -faceHeadW[2] * s)
                }
                mount = createHeadMount({
                    bone,
                    root,
