...
export interface MeshSyncOptions {
    ...
    buildGeometryMaterial: (obj: BlenderObject, textures: ResolvedTextures) => BuiltGeometryMaterial | null
}
...
export function useMeshSync({
    ...
    useEffect(() => {
        ...
        for (const r of resolved) {
            ...
            const groupKey = ownsMaterial(r.obj.name) ? `${r.cacheKey}||custom:${r.obj.name}` : r.cacheKey
        ...
        for (const [cacheKey, items] of groups) {
            ...
            if (items.length > 1) {
                ...
                const needsRebuild =
                    !entry || entry.mesh.count !== items.length || !setsEqual(entry.names, currentNames)
                ...
                if (needsRebuild) {
                    ...
                    const im = new THREE.InstancedMesh(geoMat.geometry, geoMat.material, items.length)
                    im.castShadow = true
                ...
                items.forEach((item, i) => {
                    ...
                    const matrix = new THREE.Matrix4().compose(
                        new THREE.Vector3(item.obj.position[0], item.obj.position[1], item.obj.position[2]),
                        ...
                        new THREE.Vector3(item.obj.scale[0], item.obj.scale[1], item.obj.scale[2]),
                    )
                ...
                if (!cached || cached.version !== cacheKey) {
                    ...
                    const keepMaterial = ownsMaterial(obj.name) && cached ? cached.mesh.material : undefined
                    ...
                    const mesh = new THREE.Mesh(geoMat.geometry, keepMaterial ?? geoMat.material)
                    mesh.name = obj.name
                ...
                if (cached) {
                    cached.mesh.position.set(obj.position[0], obj.position[1], obj.position[2])
