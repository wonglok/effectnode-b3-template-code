...
export function RefreshButton({ className = 'block px-3 py-1 text-white text-sm bg-blue-500 rounded-lg m-1' }) {
    ...
    return (
        <>
            <button
                ...
                className={className}
            >
                Refresh {connectionState === 'connected' ? `[Connected]` : `[Disconnected]`}
            </button>
        </>
    )
}
...
export function SyncViewer() {
    ...
    useMeshSync({
        ...
        resolveTextures: (obj: BlenderObject) => ({
            map: obj.texture ? getOrCreateTexture(obj.texture, texData, 'color') : null,
            roughnessMap: obj.roughnessMap ? getOrCreateTexture(obj.roughnessMap, texData, 'noncolor') : null,
            metalnessMap: obj.metalnessMap ? getOrCreateTexture(obj.metalnessMap, texData, 'noncolor') : null,
            normalMap: obj.normalMap ? getOrCreateTexture(obj.normalMap, texData, 'noncolor') : null,
            emissiveMap: obj.emissiveMap ? getOrCreateTexture(obj.emissiveMap, texData, 'color') : null,
        }),
    ...
    return (
        <group>
          ...
          CameraSync + NavMeshRig (runs at a later frame priority) */}
            ...
            {lights.map((light) => (
                <LightFromData key={light.name} light={light} intensityScale={ENERGY_SCALE} />
            ))}
        </group>
    )
}
